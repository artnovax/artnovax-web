      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-ivory-300">
          ...
          <button
            ...
            className="w-9 h-9 rounded-full hover:bg-ivory-200 flex items-center justify-center"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <div className="mt-4 font-serif-display text-ink text-[18px]">
                Your bag is empty
              </div>
              <p className="mt-1 text-ink/70 text-[13.5px]">
                Every purchase helps fund our programs.
              </p>
              <a
                href="/shop"
                onClick={() => setOpen(false)}
                className="cta-btn mt-5 inline-flex items-center gap-2 rounded-full bg-burgundy text-ivory px-5 py-2.5 text-[13.5px] font-semibold hover:bg-burgundy-light"
              >
              ...
              {items.map((i) => (
                <li key={i.id} className="flex gap-3 items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 inline-flex items-center gap-1 ring-1 ring-ivory-300 rounded-full">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-ivory-200"
                      >
                        <Minus className="w-3.5 h-3.5" />
                      </button>
                      <span className="px-2 text-[13px] min-w-[20px] text-center">
                        {i.qty}
                      </span>
                      <button
                        aria-label="Increase"
                  ...
                  <div className="text-right">
                    <div className="text-[14px] font-semibold text-burgundy">
                      {formatKES(i.priceNum * i.qty)}
                    </div>
                    <button
                      onClick={() => remove(i.id)}
                      aria-label="Remove"
                      className="mt-2 text-ink/60 hover:text-burgundy inline-flex items-center gap-1 text-[12px]"
                    >
        ...
        {items.length > 0 && (
          <div className="px-5 py-4 border-t border-ivory-300 bg-ivory-100">
            ...
            <div className="text-ink/60 text-[11.5px] mt-1">
              Taxes and shipping calculated at checkout.
            </div>
            <a
              href="/checkout"
              onClick={() => setOpen(false)}
              className="cta-btn mt-4 flex justify-center items-center gap-2 rounded-full bg-burgundy text-ivory px-5 py-3.5 text-[14.5px] font-semibold hover:bg-burgundy-light"
            >
