...
const iconFor = (key) => {
  if (key === "target")
    return <Target className="w-9 h-9 text-burgundy" strokeWidth={1.4} />;
  if (key === "eye")
    return <Eye className="w-9 h-9 text-burgundy" strokeWidth={1.4} />;
...
const About = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const remoteContent = await getAboutPageContent();
        if (!cancelled) setPageContent(remoteContent);
      } catch (error) {
        console.warn("Using built-in About page content.", error);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.warn("Failed to load founders:", err);
      }
    })();
  }, []);
    ...
    <div className="min-h-screen bg-ivory">
        ...
        <div className="mt-10 md:mt-14 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {founders.map((p) => (
            <a
              ...
              className="wwd-card group block"
            >
              ...
              <div className="mt-3">
                <h3 className="font-serif-display text-burgundy text-[17px] font-semibold leading-tight">
                  {p.name}
                </h3>
                <div className="text-[11px] tracking-widest text-ink/70 mt-1 uppercase">
                  {p.role}
                </div>
                <p className="mt-2 text-ink/80 text-[13px] leading-relaxed line-clamp-3">
                  {p.short}
                </p>
                <div className="mt-2 text-burgundy text-[12.5px] font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                  Read more{" "}
                  <Linkedin
      ...
      <section className="mx-auto max-w-[1180px] px-4 md:px-8 mt-12 md:mt-16 mb-16 md:mb-24">
        <div className="rounded-3xl bg-ivory-200/70 ring-1 ring-ivory-300 p-6 md:p-8 grid grid-cols-1 md:grid-cols-[auto_1fr_auto] items-center gap-6">
          ...
          <div>
            <h3 className="font-serif-display text-burgundy text-[22px] md:text-[26px] font-semibold">
              {pageContent.cta.title}
            </h3>
            <p className="text-ink/80 text-[14.5px] leading-relaxed mt-1">
              {pageContent.cta.body}
            </p>
          </div>
          <a
            href={pageContent.cta.button.href}
