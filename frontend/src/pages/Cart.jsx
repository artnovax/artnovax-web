      ...
      <section className="mx-auto max-w-[1180px] px-4 md:px-8 py-10 md:py-14">
        <div className="flex items-end justify-between border-b border-ivory-300 pb-4">
          ...
          {items.length > 0 && (
            <button
              onClick={clear}
              className="text-burgundy text-[13px] font-semibold hover:underline"
            >
              Clear cart
            </button>
          )}
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <div className="mt-5 font-serif-display text-ink text-[22px]">
              Your bag is empty
            </div>
            <p className="mt-2 text-ink/70 max-w-[420px] mx-auto">
              Explore our collection—every purchase supports ArtNovaX programs.
            </p>
            <a
              href="/shop"
              className="cta-btn mt-6 inline-flex items-center gap-2 rounded-full bg-burgundy text-ivory px-6 py-3.5 text-[14.5px] font-semibold hover:bg-burgundy-light"
            >
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] gap-8">
                ...
                <li key={i.id} className="py-5 flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[13px] text-ink/60 mt-0.5">
                      {formatKES(i.priceNum)}
                    </div>
                    <div className="mt-3 inline-flex items-center ring-1 ring-ivory-300 rounded-full">
                      <button
                        onClick={() => setQty(i.id, i.qty - 1)}
                        className="w-8 h-8 flex items-center justify-center hover:bg-ivory-200 rounded-full"
                      >
                        <Minus className="w-4 h-4" />
                      </button>
                      <span className="px-3 min-w-[24px] text-center text-[14px]">
                        {i.qty}
                      </span>
                      <button
                        onClick={() => setQty(i.id, i.qty + 1)}
                  ...
                  <div className="text-right">
                    <div className="text-[16px] font-semibold text-burgundy">
                      {formatKES(i.priceNum * i.qty)}
                    </div>
                    <button
                      onClick={() => remove(i.id)}
                      className="mt-2 text-ink/60 hover:text-burgundy inline-flex items-center gap-1 text-[13px]"
                    >
            ...
            <aside className="h-fit rounded-2xl bg-ivory-100 ring-1 ring-ivory-300 p-6">
              ...
              <a
                href="/checkout"
                className="cta-btn mt-5 flex justify-center items-center gap-2 rounded-full bg-burgundy text-ivory px-5 py-3.5 text-[14.5px] font-semibold hover:bg-burgundy-light"
              >
