  ...
  <div className="rounded-3xl overflow-hidden ring-1 ring-ivory-300 bg-ivory-100">
    <div className="relative aspect-[4/5] w-full">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-5 text-ivory">
        <div className="text-[10.5px] tracking-widest opacity-85">
          {ev.location || ""}
        </div>
    ...
    {(ev.partners || []).length > 0 && (
      <div className="px-4 py-3 bg-ivory">
        <div className="text-[10.5px] tracking-widest text-ink/60">
          Partners
        </div>
...
const downloadEventPoster = async (ev) => {
  const W = 1080,
    H = 1350; // Instagram Story-friendly 4:5
  ...
  ctx.fillText(
    "where art heals, tech empowers, & minds transform.",
    80,
    H - 66,
  );
  ctx.font = "600 22px Inter, Arial, sans-serif";
...
const ShareRow = ({ ev }) => {
  ...
  const downloadPoster = async () => {
    ...
    setDownloading(true);
    try {
      await downloadEventPoster(ev);
    } catch (err) {
      console.warn("Poster download failed:", err);
    } finally {
  ...
  return (
    <div className="mt-6 rounded-2xl bg-ivory-100 ring-1 ring-ivory-300 p-4">
      <div className="text-[12.5px] tracking-widest text-ink/60 font-semibold uppercase">
        Share this event
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          onClick={nativeShare}
          data-testid="event-share-native"
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          data-testid="event-share-whatsapp"
          className="cta-btn inline-flex items-center gap-2 rounded-full ring-1 ring-ivory-300 bg-ivory px-4 py-2 text-[13px] font-semibold text-ink hover:ring-burgundy/40"
        >
          WhatsApp
        </a>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          data-testid="event-share-instagram"
          className="cta-btn inline-flex items-center gap-2 rounded-full ring-1 ring-ivory-300 bg-ivory px-4 py-2 text-[13px] font-semibold text-ink hover:ring-burgundy/40"
        >
          Instagram
        </a>
        ...
        >
          {copied ? (
            <>
              <Check className="w-4 h-4 text-burgundy" /> Copied
            </>
          ) : (
            <>
        ...
        <button
          ...
          data-testid="event-download-poster"
          disabled={downloading}
          className="cta-btn inline-flex items-center gap-2 rounded-full ring-1 ring-ivory-300 bg-ivory px-4 py-2 text-[13px] font-semibold text-ink hover:ring-burgundy/40 disabled:opacity-60"
        >
          <Download className="w-4 h-4" />{" "}
          {downloading ? "Rendering…" : "Download poster"}
        </button>
      </div>
      <p className="mt-2 text-ink/60 text-[12px]">
        Tip: on your phone, tap <b>Share</b> to send this directly to Instagram
        Stories or WhatsApp. The <b>Download poster</b> saves a 1080×1350 image
        perfect for stories or print.
      </p>
    </div>
  );
};
...
const EventDetail = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-ivory">
        ...
        <section className="mx-auto max-w-[720px] px-6 py-24 text-center text-ink/60">
          Loading event…
        </section>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen bg-ivory">
      ...
      <section className="mx-auto max-w-[1200px] px-4 md:px-8 pt-8 md:pt-12 pb-16 md:pb-24">
        <a
          href="/events"
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)] gap-8">
          ...
          {/* Details */}
          <div>
            <div className="inline-block bg-burgundy text-ivory text-[10.5px] font-semibold tracking-widest px-2 py-1 rounded">
              {(ev.status || "UPCOMING").toUpperCase()}
            </div>
            <h1 className="mt-3 font-serif-display text-burgundy text-[36px] md:text-[48px] leading-[1.05] font-semibold">
              {ev.title}
            </h1>
            {ev.subtitle && (
              <div className="text-ink/70 text-[15px] mt-1">{ev.subtitle}</div>
            )}
            ...
            {tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {tags.map((t) => (
                  <span
                    key={t}
                    className="text-[11.5px] font-semibold text-burgundy bg-burgundy/10 rounded-full px-3 py-1"
                  >
                    {t}
                  </span>
                ))}
            ...
            {ev.body && (
              <div className="mt-6 text-ink/85 text-[15.5px] leading-[1.75] whitespace-pre-wrap">
                {ev.body}
              </div>
            )}
              ...
              <div className="mt-6 rounded-2xl bg-ivory-100 ring-1 ring-ivory-300 p-4">
                <div className="flex items-baseline justify-between gap-4">
                  ...
                  <div className="text-[13.5px] text-ink/70">
                    ...
                    {ev.is_full && (
                      <span className="ml-2 text-[11.5px] font-semibold text-burgundy bg-burgundy/10 rounded-full px-2 py-0.5">
                        Waitlist open
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-2 h-1.5 rounded-full bg-ivory-300 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round(((ev.registered_count || 0) / ev.capacity) * 100))}%`,
                    }}
                  />
                </div>
                {!ev.is_full && typeof ev.spots_left === "number" && (
                  <div className="mt-2 text-[12.5px] text-ink/60">
                    {ev.spots_left} {ev.spots_left === 1 ? "spot" : "spots"}{" "}
                    left
                  </div>
                )}
              </div>
            ...
            {ev.status !== "past" && (
              <div className="mt-8 flex flex-wrap gap-3">
                <a
                  href={`/events/${ev.slug || ev.id}/register`}
                  className="cta-btn inline-flex items-center gap-2 rounded-full bg-burgundy text-ivory px-6 py-3.5 text-[14.5px] font-semibold hover:bg-burgundy-light"
                >
                  <Ticket className="w-4 h-4" />{" "}
                  {ev.is_full ? "Join the waitlist" : "Register now"}
                </a>
                <a
